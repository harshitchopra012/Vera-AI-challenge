...
try:
    ...
    INTELLIGENCE_IMPORT_ERROR = ""
except Exception as exc:  # pragma: no cover - keeps the deployed API bootable.
    INTELLIGENCE_IMPORT_ERROR = str(exc)
    ...
    def intelligent_message(
        ...
        return compose_body(category, merchant, trigger, customer), {
            "quality_issues": ["intelligence_import_error"],
            "selection_score": 0,
            "category_fit": 0,
            "merchant_fit": 0,
            "trigger_relevance": 0,
            "candidate_count": 0,
            "import_error": INTELLIGENCE_IMPORT_ERROR,
        }
...
def run_server(host: str = "0.0.0.0", port: int = 8080) -> None:
    if INTELLIGENCE_IMPORT_ERROR:
        print(f"Intelligence layer unavailable, using fallback composers: {INTELLIGENCE_IMPORT_ERROR}")
    server = ThreadingHTTPServer((host, port), StdlibHandler)
    print(f"ContextCraft Vera listening on http://{host}:{port}")
    server.serve_forever()


def runtime_port(default: int = 8080) -> int:
