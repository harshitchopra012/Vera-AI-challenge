...
def refine_selected_insight(
    ...
    agg = merchant.get("customer_aggregate", {})
    offer = best_offer(category, merchant)
    current_insight = safe_text(refined.get("insight"))
    current_action = safe_text(refined.get("action"))

    if customer:
        cname = safe_text(customer.get("identity", {}).get("name"), "this customer").split("(")[0].strip()
        if kind == "recall_due":
            slots = [slot.get("label") for slot in payload.get("available_slots", []) if slot.get("label")]
            refined["insight"] = f"{cname}'s dental recall is due after the {payload.get('last_service_date')} visit; the timing is warm, not cold outreach."
            refined["action"] = f"Offer {' / '.join(slots[:2]) if slots else 'this week'} with {offer}"
        elif kind == "chronic_refill_due":
            meds = ", ".join(payload.get("molecule_list", []))
            refined["insight"] = f"{cname}'s pharmacy refill for {meds} runs out on {safe_text(payload.get('stock_runs_out_iso'))[:10]}."
            refined["action"] = category_action(category, merchant, trigger, offer)
        elif kind == "customer_lapsed_hard":
            refined["insight"] = f"{cname} has been away {payload.get('days_since_last_visit')} days; the last fitness goal was {clean_label(payload.get('previous_focus'))}."
            refined["action"] = category_action(category, merchant, trigger, offer)
        else:
            refined["insight"] = customer_generic_insight(category, merchant, trigger, customer)
            refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "perf_dip":
        metric = clean_label(payload.get("metric") or "performance").title()
        refined["insight"] = f"{metric} is down {pct(payload.get('delta_pct'), signed=True)} in {payload.get('window', '7d')}; current is {perf.get(clean_label(payload.get('metric'))) or perf.get('calls')}, so demand is leaking before conversion."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "perf_spike":
        metric = clean_label(payload.get("metric") or "performance").title()
        current = perf.get(clean_label(payload.get("metric"))) or perf.get("calls")
        baseline = payload.get("vs_baseline")
        refined["insight"] = f"{metric} is up {pct(payload.get('delta_pct'), signed=True)} in {payload.get('window', '7d')} (current {current}; baseline {baseline}); demand is warm enough to convert now."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "competitor_opened":
        refined["insight"] = f"Competitor {payload.get('competitor_name')} opened {payload.get('distance_km')} km away with {payload.get('their_offer')}."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "review_theme_emerged":
        refined["insight"] = f"{payload.get('occurrences_30d')} reviews mention {clean_label(payload.get('theme'))}; that is now visible enough to affect trust."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "gbp_unverified":
        label = "Pharmacy GBP" if slug == "pharmacies" else f"{(slug or 'Business').title()} GBP"
        refined["insight"] = f"{label} is unverified despite {perf.get('views')} monthly views; verification can unlock about {pct(payload.get('estimated_uplift_pct'))} more profile actions."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "supply_alert":
        batches = ", ".join(payload.get("affected_batches", []))
        refined["insight"] = f"{payload.get('molecule')} medicine recall affects batches {batches}; this pharmacy has {agg.get('chronic_rx_count')} chronic-Rx customers."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "category_seasonal" and slug == "pharmacies":
        trends = ", ".join(clean_label(t) for t in payload.get("trends", [])[:4])
        refined["insight"] = f"Pharmacy seasonal demand is shifting now: {trends}."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "festival_upcoming":
        refined["insight"] = f"{payload.get('festival')} is {payload.get('days_until')} days away; early {slug or 'merchant'} booking prep beats last-week discounting."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind == "active_planning_intent":
        refined["insight"] = f"The merchant already showed intent on {clean_label(payload.get('intent_topic'))}: '{safe_text(payload.get('merchant_last_message'))}'."
        refined["action"] = category_action(category, merchant, trigger, offer)
    elif kind in {"research_digest", "cde_opportunity", "regulation_change"}:
        item = find_digest_item(category, trigger) or {}
        source = safe_text(item.get("source"), "category digest")
        title = safe_text(item.get("title"), current_insight)
        if kind == "research_digest" and agg.get("high_risk_adult_count"):
            refined["insight"] = f"{source}: {title}; it maps to {agg.get('high_risk_adult_count')} high-risk adult patients at this clinic."
        elif kind == "cde_opportunity":
            refined["insight"] = f"{source}: {title}; {payload.get('credits')} CDE credits and fee {clean_label(payload.get('fee'))}."
        elif kind == "regulation_change":
            refined["insight"] = f"{source}: {title}; deadline is {payload.get('deadline_iso')}."
        refined["action"] = current_action or category_action(category, merchant, trigger, offer)

    if not safe_text(refined.get("action")):
        refined["action"] = category_action(category, merchant, trigger, offer)
    if not safe_text(refined.get("insight")):
        refined["insight"] = current_insight

    refined = annotate_selection_scores(refined, category, merchant, trigger)
    original = annotate_selection_scores(insight, category, merchant, trigger)
    if refined.get("selection_score", 0) < original.get("selection_score", 0):
        return original
    return refined
...
def final_message_polish(
    ...
    hook_bank = {
        ...
        "opportunity": [
            ...
            f"{salutation}, opportunity:",
        ],
        "warm": [
            f"{salutation}, follow-up is warm:",
            f"{salutation}, timely customer moment:",
            f"{salutation}, easy customer win:",
        ],
        "curious": [
            f"{salutation}, signal:",
...
def intelligent_message(
    ...
    best = refine_selected_insight(ranked[0], category, merchant, trigger, customer)
